...
from glowtts_modules import LayerNorm
...
class CouplingBlock(nn.Module):
  def __init__(self, in_channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0, p_dropout=0, sigmoid_scale=False):
    ...
    self.wn = glowtts_modules.WN(in_channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels, p_dropout)
...
class MultiHeadAttention(nn.Module):
  ...
  def _get_relative_embeddings(self, relative_embeddings, length):
    ...
    if pad_length > 0:
      padded_relative_embeddings = F.pad(
          relative_embeddings,
          glowtts_commons.convert_pad_shape([[0, 0], [pad_length, pad_length], [0, 0]]))
    else:
  ...
  def _relative_position_to_absolute_position(self, x):
    ...
    x = F.pad(x, glowtts_commons.convert_pad_shape([[0,0],[0,0],[0,0],[0,1]]))

    # Concat extra elements so to add up to shape (len+1, 2*len-1).
    x_flat = x.view([batch, heads, length * 2 * length])
    x_flat = F.pad(x_flat, glowtts_commons.convert_pad_shape([[0,0],[0,0],[0,length-1]]))
  ...
  def _absolute_position_to_relative_position(self, x):
    ...
    batch, heads, length, _ = x.size()
    # padd along column
    x = F.pad(x, glowtts_commons.convert_pad_shape([[0, 0], [0, 0], [0, 0], [0, length-1]]))
    x_flat = x.view([batch, heads, length**2 + length*(length -1)])
    # add 0's in the beginning that will skew the elements after reshape
    x_flat = F.pad(x_flat, glowtts_commons.convert_pad_shape([[0, 0], [0, 0], [length, 0]]))
